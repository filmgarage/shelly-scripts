/**
 * Shelly Pro 3EM - Simple Meter Synchronization Script
 * 
...
let CONFIG = {
  ...
  emId: 0,                // EM component ID (usually 0 for Pro 3EM)
...
function syncToShelly() {
  ...
  // Get current EMData status
  Shelly.call("EMData.GetStatus", { id: CONFIG.emId }, function(result, error_code, error_message) {
    if (error_code !== 0) {
      print("Error getting EMData status:", error_message);
      return;
    }
    
    // Get current total energy (convert to kWh)
    let shellyTotal = (result.total_act || 0) / 1000;
...
function updateMeterTotal() {
  // Get EMData status (accumulated energy values)
  Shelly.call("EMData.GetStatus", { id: CONFIG.emId }, function(result, error_code, error_message) {
    if (error_code !== 0) {
      print("Error getting EMData status:", error_message);
      return;
    ...
    // Get Shelly total energy (in Wh, convert to kWh)
    let shellyTotal = (result.total_act || 0) / 1000;
...
print("Shelly Pro 3EM Meter Synchronization Script started!");
